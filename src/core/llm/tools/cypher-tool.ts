...
export const createCypherTool = (
  backend: Pick<GraphRAGBackend, 'executeQuery' | 'search'>,
) =>
  tool(
    async ({ query, cypher }: { query?: string; cypher: string }) => {
      try {
        if (cypher.includes('{{QUERY_VECTOR}}')) {
          ...
          try {
            const semanticResults = await backend.search(query, { limit: 10, mode: 'semantic' });
        ...
        const results = await backend.executeQuery(cypher);
