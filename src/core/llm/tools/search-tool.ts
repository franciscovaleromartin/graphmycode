...
import { z } from 'zod';
import type { GraphRAGBackend, EnrichedSearchResult } from './types';
...
export const createSearchTool = (backend: Pick<GraphRAGBackend, 'search'>) =>
  tool(
    async ({
      query,
      limit,
      ...
      try {
        searchResults = await backend.search(query, { limit: k, enrich: true });
      } catch {
      ...
      const results: ResultInfo[] = searchResults.slice(0, k).map((r, i) => {
        ...
        return {
          ...
          score,
          connections,
          clusterLabel: (r.cluster as string) || 'Unclustered',
          processes: (r.processes || []).filter((p) => p.id && p.label),
        };
      });
      ...
      const sortedProcesses = Array.from(processMap.entries()).sort(
        (a, b) => b[1].entries.length - a[1].entries.length,
      );

      const lines: string[] = [`Found ${searchResults.length} matches grouped by process:`, ''];
